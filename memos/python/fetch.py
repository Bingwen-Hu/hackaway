...
def extra_info(soup):
    rs = soup.find_all('dl', {'class': 'basicInfo-block'})
    left, right = rs
    leftd = extra_info_helper(left)
    rightd = extra_info_helper(right)
    return leftd, rightd

def extra_info_helper(info):
    pat = re.compile(r'\xa0')
    info = pat.sub("", info.get_text().strip())
    pat = re.compile(r"\n+")
    info = re.split(pat, info)
    dicts = {}
...
def test_main():
    """ Problem may occur in params list or string?"""
    global leaders
    leaders_test = test_leaders(leaders)
    params = main(leaders_test)
    return params
#==============================================================================
