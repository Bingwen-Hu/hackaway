...
# run number of [n] clients
# python3 server.py client [n]
...
def printer(start_time, *args, **kwargs):
    '''Simple function to print a message prefixed with the time relative
    to the given start time'''
    print('{0:.1f}'.format(time.time() - start_time), *args, **kwargs)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'server':
        server = asyncio.start_server(handle_connection, HOST, PORT)
        running_server = loop.run_until_complete(server)

        try:
            result = loop.call_later(20, loop.stop)
            loop.run_forever()
