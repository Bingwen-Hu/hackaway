...
from eatable import coco


class Joint(enum.IntEnum):
    """Joint type for pose estimation task, we define our own keypoints 
    order and joint type, which is different from COCO's. 
...
class KeyPointMixin(object):
    ...
    def im_letterbox(im, tsize, stride):
        ...
        if im_h < im_w:
            ...
            # pad width to be multiple to stride
            surplus = (stride - (im_w % stride)) % stride
            im_w += surplus
            im_h = tsize
        else:
            im_h = round(tsize / im_w * im_h)
            # pad height to be multiple to stride
    ...
    def plot_ignore_mask(im, mask, color=(0, 0, 1)):
        ...
        im_masked = im * mask
        # keep certain channels according to color, change 1s to 255s
        for i in range(3):
...
class KeyPointTrain(coco.COCO, KeyPointMixin):
