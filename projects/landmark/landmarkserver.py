...
# 通过url指定不同的检测模式
@routes.view('/faceapi/{mode}/detect')
class RecognizeView(web.View):
    async def post(self):
        ...
        info = landmark.detect(filename, mode=self.request.match_info['mode'])
        if info:
            result = {
                'state': 10000,
                'message': 'succeed',
...
# 通过gunicorn来运行：gunicorn landmarkserver:landmark_server --worker-class aiohttp.GunicornWebWorker
async def landmark_server():
