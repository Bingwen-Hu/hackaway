...
def batch_norm_relu(inputs, is_training, data_format):
    """Perfomrs a batch normalization followed by a ReLU"""
...
def fixed_padding(inputs, kernel_size, data_format):
    ...
    if data_format == 'channels_first':
        padded_inputs = tf.pad(inputs, [[0, 0], [0, 0],
                                        [pad_begin, pad_end], [pad_begin, pad_end]])
    else:
...
def bottleneck_block(inputs, filters, is_training, projection_shortcut,
    ...
    # The projection shortcut should come after the first batch norm and ReLU
    # since it performs a 1x1 convolutions.
    if projection_shortcut is not None:
        shortcut = projection_shortcut(inputs)
...
def block_layer(inputs, filters, block_fn, blocks, strides, is_training, name, data_format):
    ...
    return tf.identity(inputs, name)  # bother for the name


# main network construct here
def cifar10_resnet_v2_generator(resnet_size, num_classes, data_format=None):
    ...
    Returns:
        ...
        return the output tensor of the ResNet model.
...
def imagenet_resnet_v2_generator(block_fn, layers, num_classes,
    ...
    def model(inputs, is_training):
        ...
        inputs = conv2d_fixed_padding(inputs=inputs, filters=64, kernel_size=7,
